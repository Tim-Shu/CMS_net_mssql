...
namespace QJcms.BLL
{
	...
	public partial class channel_category
	{
		...
		{
            dal = new DAL.channel_category(QJKey.DatabasePrefix);
        }
		#region 基本方法========================================
        ...
        public bool Exists(string build_path)
        {
            ...
            if (DirPathExists(siteConfig.webpath + "/" + QJConst.DIRECTORY_REWRITE_ASPX + "/", build_path))
            {
                return true;
            }
            //与频道名称是否同名
            if (new DAL.channel(QJKey.DatabasePrefix).Exists(build_path))
            {
                return true;
            }
            //与频道分类生成目录是否同名
		...
		public bool Update(Model.channel_category model)
		{
            ...
            if (dal.Update(model))
            {
                ...
                {
                    //更改频道分类对应的目录名称
                    Utils.MoveDirectory(siteConfig.webpath + QJConst.DIRECTORY_REWRITE_ASPX + "/" + oldModel.build_path,
                        siteConfig.webpath + QJConst.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                    Utils.MoveDirectory(siteConfig.webpath + QJConst.DIRECTORY_REWRITE_HTML + "/" + oldModel.build_path,
                        siteConfig.webpath + QJConst.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                }
                //修改对应的导航
                BLL.navigation navBLL = new navigation();
                Model.navigation navModel = navBLL.GetModel("channel_" + oldModel.build_path);
                navBLL.Update("channel_" + oldModel.build_path, "channel_" + model.build_path, model.title, navModel.is_lock);
        ...
        {
            List<Model.channel_category> modelList = new List<QJcms.Model.channel_category>();
            ...
            if (rowsCount > 0)
            {
                ...
                {
                    model = new QJcms.Model.channel_category();
