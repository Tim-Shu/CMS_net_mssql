...
namespace QJcms.BLL
{
    public class url_rewrite
    {
        ...
        {
            Hashtable ht = CacheHelper.Get<Hashtable>(QJConst.CACHE_SITE_URLS);
            if (ht == null)
            {
                CacheHelper.Insert(QJConst.CACHE_SITE_URLS, dal.GetList(), Utils.GetXmlMapPath(QJConst.FILE_URL_XML_CONFING));
                ht = CacheHelper.Get<Hashtable>(QJConst.CACHE_SITE_URLS);
            }
            return ht;
        }
        ...
        {
            List<Model.url_rewrite> ls = CacheHelper.Get<List<Model.url_rewrite>>(QJConst.CACHE_SITE_URLS_LIST);
            if (ls == null)
            {
                CacheHelper.Insert(QJConst.CACHE_SITE_URLS_LIST, dal.GetList(""), Utils.GetXmlMapPath(QJConst.FILE_URL_XML_CONFING));
                ls = CacheHelper.Get<List<Model.url_rewrite>>(QJConst.CACHE_SITE_URLS_LIST);
            }
            return ls;
        }
