...
namespace QJcms.Web.UI
{
    public class HtmlBuilder
    {
        ...
        {
            string lang = QJRequest.GetQueryString("lang");
            string name = QJRequest.GetQueryString("name");
            string type = QJRequest.GetQueryString("type");
                    ...
                    if (modeltrewrite.url_rewrite_items.Count > 0)
                    {
                        if (modeltrewrite.channel == string.Empty || modeltrewrite.channel == name)
                        {
                            ...
                            foreach (Model.url_rewrite_item item in modeltrewrite.url_rewrite_items)
                            {
                                ...
                                {
                                    linkurl = string.Format("{0}/{1}/{2}", QJConst.DIRECTORY_REWRITE_ASPX, lang, modeltrewrite.page);
                                    string HTMLPattern = string.Format("{0}/{1}/{2}", QJConst.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(item.pattern, config.staticextension)); //替换扩展名
                                    if (sbjson.ToString().Length > 1)
                                        sbjson.Append(",");
        ...
        private string GetArticleIndexUrlList(string lang, string channelname, string page, string pattern, string path, string querystring, int pagesize)
        {
            StringBuilder sburl = new StringBuilder();
                    ...
                    for (int q = 1; q <= pageindex; q++)
                    {
                        ...
                        string linkurl = string.Format("{0}/{1}/{2}?{3}", QJConst.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                        string HTMLPattern = string.Format("{0}/{1}/{2}", QJConst.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, "0", q), config.staticextension)); //替换扩展名
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    for (int q = 1; q <= pageindex; q++)
                    {
                        ...
                        string linkurl = string.Format("{0}/{1}/{2}?{3}", QJConst.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                        string HTMLPattern = string.Format("{0}/{1}/{2}", QJConst.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, dt.Rows[i]["id"].ToString(), q), config.staticextension)); //替换扩展名
                        if (!string.IsNullOrEmpty(sburl.ToString()))
                            sburl.Append(",");
                        sburl.AppendFormat(urlstr, config.webpath, lang, linkurl.Replace("&","^"), HTMLPattern);
                    }
        ...
        private string GetCategoryUrlList(string lang, string channelname, string page, string pattern, string path, string querystring, int pagesize)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string linkurl = string.Format("{0}/{1}/{2}?{3}", QJConst.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                    string HTMLPattern = string.Format("{0}/{1}/{2}", QJConst.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, dt.Rows[i]["id"].ToString()), config.staticextension)); //替换扩展名
                    if (!string.IsNullOrEmpty(sburl.ToString()))
                        sburl.Append(",");
        ...
        private string GetDetailUrlList(string lang, string channelname, string page, string pattern, string path, string querystring)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string linkurl = string.Format("{0}/{1}/{2}?{3}", QJConst.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                    string HTMLPattern = string.Format("{0}/{1}/{2}", QJConst.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, strvalue), config.staticextension)); //替换扩展名
                    if (!string.IsNullOrEmpty(sburl.ToString()))
                        sburl.Append(",");
                    sburl.AppendFormat(urlstr, config.webpath, lang, linkurl, HTMLPattern);
        ...
        {
            string lang = QJRequest.GetQueryString("lang");
            string aspx_filename = QJRequest.GetQueryString("aspx_filename");
            string catalogue = QJRequest.GetQueryString("catalogue");
