...
namespace QJcms.Web.UI.Page
{
    public partial class payment : Web.UI.BasePage
    {
        ...
        {
            //取得处事类型
            action = QJRequest.GetString("action");
            order_no = QJRequest.GetString("order_no");
            if (order_no.ToUpper().StartsWith("R")) //充值订单
            {
                order_type = QJEnums.AmountTypeEnum.Recharge.ToString().ToLower();
            }
            else if (order_no.ToUpper().StartsWith("B")) //商品订单
            {
                order_type = QJEnums.AmountTypeEnum.BuyGoods.ToString().ToLower();
            }
