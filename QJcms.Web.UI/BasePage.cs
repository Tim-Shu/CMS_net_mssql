...
namespace QJcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        {
            //如果Session为Null
            if (HttpContext.Current.Session[QJConst.SESSION_USER_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Utils.GetCookie(QJConst.COOKIE_USER_NAME_REMEMBER, "QJcms");
                string password = Utils.GetCookie(QJConst.COOKIE_USER_PWD_REMEMBER, "QJcms");
                if (username != "" && password != "")
                {
                    ...
                    if (model != null)
                    {
                        HttpContext.Current.Session[QJConst.SESSION_USER_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        public Model.users GetUserInfo()
        {
            if (IsUserLogin())
            {
                Model.users model = HttpContext.Current.Session[QJConst.SESSION_USER_INFO] as Model.users;
                if (model != null)
                {
        ...
        private string GetFirstPath(string requestPath)
        {
            ...
            if (requestPath.StartsWith(config.webpath + QJConst.DIRECTORY_REWRITE_ASPX + "/"))
            {
                indexNum = (config.webpath + QJConst.DIRECTORY_REWRITE_ASPX + "/").Length;
            }
