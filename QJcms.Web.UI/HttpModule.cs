...
namespace QJcms.Web.UI
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            //{
                ...
                if (IsDirExist(QJConst.CACHE_SITE_DIRECTORY, siteConfig.webpath, siteConfig.webpath, requestPath))
                {
                    return;
                }
                ...
                if (siteConfig.staticstatus == 0)
                {
                    ...
                    foreach (Model.url_rewrite model in SiteUrls.GetSiteUrls().Urls)
                    {
                        ...
                        if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                        {
                            ...
                            if (model.type == QJConst.DIRECTORY_REWRITE_PLUGIN)
                            {
                                context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                    siteConfig.webpath, QJConst.DIRECTORY_REWRITE_ASPX, QJConst.DIRECTORY_REWRITE_PLUGIN, requestPath));
                                return;
                            }
                            else
                            {
                                ...
                                context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                    siteConfig.webpath, QJConst.DIRECTORY_REWRITE_ASPX, defaultPath, requestPath));
                                return;
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    foreach (Model.url_rewrite model in SiteUrls.GetSiteUrls().Urls)
                    {
                            ...
                            Utils.GetUrlExtension(model.page, siteConfig.staticextension) == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                        {
                            ...
                            {
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                    siteConfig.webpath, QJConst.DIRECTORY_REWRITE_ASPX, QJConst.DIRECTORY_REWRITE_PLUGIN, model.page));
                                return;
                            }
                            else
                            {
                                ...
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}", siteConfig.webpath, QJConst.DIRECTORY_REWRITE_ASPX, defaultPath, model.page));
                                return;
                            }
                        }
                        //遍历URL字典的子节点
                        foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                        {
                                ...
                                || (model.page == "index.aspx" && Regex.IsMatch(requestPath, string.Format("^{0}{1}$", patternPath, item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                            {
                                ...
                                if (siteConfig.staticstatus == 2 && defaultPath != QJConst.DIRECTORY_REWRITE_MOBILE &&
                                    (model.channel.Length > 0 || model.page.ToLower() == "index.aspx")) //频道页
                                {
                                    if (domainPath == string.Empty && categoryPath != string.Empty)
                                    {
                                        context.RewritePath(siteConfig.webpath + QJConst.DIRECTORY_REWRITE_HTML + Utils.GetUrlExtension(requestPath, siteConfig.staticextension, true));
                                    }
                                    else
                                    {
                                        context.RewritePath(siteConfig.webpath + QJConst.DIRECTORY_REWRITE_HTML + "/" + defaultPath + Utils.GetUrlExtension(requestPath, siteConfig.staticextension, true));
                                    }
                                    return;
                                }
                                else if (model.type == QJConst.DIRECTORY_REWRITE_PLUGIN) //插件页
                                {
                                    ...
                                    context.RewritePath(string.Format("{0}{1}/{2}/{3}", siteConfig.webpath, QJConst.DIRECTORY_REWRITE_ASPX, QJConst.DIRECTORY_REWRITE_PLUGIN, model.page),
                                ...
                                else //其它
                                {
                                    string queryString = Regex.Replace(requestPath, string.Format("{0}{1}", patternPath, newPattern),
                                        item.querystring, RegexOptions.None | RegexOptions.IgnoreCase);
                                    context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                        siteConfig.webpath, QJConst.DIRECTORY_REWRITE_ASPX, defaultPath, model.page), string.Empty, queryString);
                                    return;
                                }
                            }
                        }
    ...
    public class SiteUrls
    {
        ...
        {
            SiteUrls _cache = CacheHelper.Get<SiteUrls>(QJConst.CACHE_SITE_HTTP_MODULE);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(QJConst.CACHE_SITE_HTTP_MODULE, new SiteUrls(), Utils.GetXmlMapPath(QJConst.FILE_URL_XML_CONFING));
                    instance = CacheHelper.Get<SiteUrls>(QJConst.CACHE_SITE_HTTP_MODULE);
                }
            }
            return instance;
        }
    ...
    public class SiteDomains
    {
        ...
        {
            SiteDomains _cache = CacheHelper.Get<SiteDomains>(QJConst.CACHE_SITE_HTTP_DOMAIN);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(QJConst.CACHE_SITE_HTTP_DOMAIN, new SiteDomains(), 10);
                    instance = CacheHelper.Get<SiteDomains>(QJConst.CACHE_SITE_HTTP_DOMAIN);
                }
            }
            return instance;
        }
