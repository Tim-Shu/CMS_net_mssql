...
namespace QJcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        /// <param name="category_id">类别ID</param>
        /// <returns>Model.category</returns>
        protected Model.article_category get_category_model(string call_index)
        {
            return new BLL.article_category().GetModel(call_index);
        }
        ...
        /// <returns>DataTable</returns>
        protected DataTable get_category_list(string channel_name, string strwhere)
        {
            return new BLL.article_category().GetList(strwhere, channel_name);
        }
        ...
        /// <returns></returns>
        protected int get_first_parent_id(int category_id)
        {
            int pid = 0;
            if (category_id > 0)
            {
                Model.article_category model = new BLL.article_category().GetModel(category_id);
                pid = Utils.StrToInt(model.class_list.Split(',')[1], 0);
            }
            return pid;
        }
        ...
        private void LoopChannelMenu(StringBuilder strTxt, string urlKey, int category_id)
        {
            ...
            strTxt.Append("&nbsp;&gt;&nbsp;<a href=\"" + linkurl(urlKey, category_id) + "\">" + bll.GetTitle(category_id) + "</a>");
        }
        #endregion
    }
}
