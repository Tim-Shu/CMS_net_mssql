...
namespace QJcms.Web.UI
{
    public class UpLoad
    {
        ...
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater, int channel_id)
        {
            try
            {
                ...
                string upLoadPath = GetUpLoadPath(channel_id); //上传目录相对路径
                ...
                if (!isThumbnail)
                {
                    newThumbnailPath = upLoadPath + newFileName;
                }
                ...
                //检查文件大小是否合法
                if (!CheckFileSize(fileExt, fileSize, channel_id))
                {
                ...
                if (IsImage(fileExt))
                {
                    ...
                    {
                        if (this.siteConfig.imgmaxheight > 0 || this.siteConfig.imgmaxwidth > 0)
                        {
        ...
        public string remoteSaveAs(string fileUri,int channelID)
        {
            ...
            string newFileName = Utils.GetRamCode() + "." + fileExt; //随机生成新的文件名
            string upLoadPath = GetUpLoadPath(channelID); //上传目录相对路径
            string fullUpLoadPath = Utils.GetMapPath(upLoadPath); //上传目录的物理路径
            string newFilePath = upLoadPath + newFileName; //上传后的路径
            //检查上传的物理路径是否存在，不存在则创建
            if (!Directory.Exists(fullUpLoadPath))
        ...
        private string GetUpLoadPath(int channel_id)
        {
            string path = siteConfig.webpath + siteConfig.filepath + "/"; //站点目录+上传目录

            if (channel_id > 0)
            {
                path += new BLL.channel().GetChannelName(channel_id) + "/";
            }
        ...
        {
            //判断是否开启水印
            if (this.siteConfig.watermarktype > 0 && this.siteConfig.watermarkstatus == 1)
            {
                //判断是否可以打水印的图片类型
        ...
        private bool CheckFileSize(string _fileExt, int _fileSize, int channel_id)
        {
            Model.channel channelModel = new BLL.channel().GetModel(channel_id);
            //判断是否为图片文件
            if (IsImage(_fileExt))
            {
                if (channelModel != null)
                {
                    switch (channelModel.img_size)
                    {
                        case 0:
                            if (this.siteConfig.imgsize > 0 && _fileSize > this.siteConfig.imgsize * 1024) { return false; }
                            break;
                        case 1:
                            return true;
                        default:
                            if (_fileSize > channelModel.img_size * 1024) { return false; }
                            break;
                    }
                }
                else
            ...
            else
            {
                if (channelModel != null)
                {
                    switch (channelModel.attach_size)
                    {
                        case 0:
                            if (this.siteConfig.attachsize > 0 && _fileSize > this.siteConfig.attachsize * 1024) { return false; }
                            break;
                        case 1:
                            return true;
                        default:
                            if (_fileSize > channelModel.attach_size * 1024) { return false; }
                            break;
                    }
                }
                else
        ...
        private void CutImages(int cutType)
        {
            switch (cutType)
            {
