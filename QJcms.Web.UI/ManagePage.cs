...
namespace QJcms.Web.UI
{
    public class ManagePage : System.Web.UI.Page
    {
        ...
        private void ManagePage_Load(object sender, EventArgs e)
        {
            ...
            if (!IsAdminLogin())
            {
                Response.Write("<script>parent.location.href='" + siteConfig.webpath + siteConfig.webmanagepath + "/qijudl.aspx'</script>");
                Response.End();
            }
        }
        ...
        {
            //如果Session为Null
            if (Session[QJConst.SESSION_ADMIN_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string adminname = Utils.GetCookie("AdminName", "QJcms");
                string adminpwd = Utils.GetCookie("AdminPwd", "QJcms");
        ...
        public Model.manager GetAdminInfo()
        {
            if (IsAdminLogin())
            {
                Model.manager model = Session[QJConst.SESSION_ADMIN_INFO] as Model.manager;
                if (model != null)
                {
                    return model;
                }
        ...
        public bool IsAdminLevel(string nav_name, string action_type)
        {
        ...
        public bool AddAdminLog(string action_type, string remark)
        {
            ...
            {
                Model.manager model = GetAdminInfo();
                int newId = new BLL.manager_log().Add(model.role_type, model.id, model.user_name, action_type, remark);
                if (newId > 0)
                {
                    return true;
                }
