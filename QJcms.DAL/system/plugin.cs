...
namespace QJcms.DAL
{
    public partial class plugin
    {
        ...
        public Model.plugin GetInfo(string dirPath)
        {
            ...
            if (!File.Exists(dirPath + QJConst.FILE_PLUGIN_XML_CONFING))
                return aboutInfo;

            XmlDocument xml = new XmlDocument();
            xml.Load(dirPath + QJConst.FILE_PLUGIN_XML_CONFING);
            try
            {
        ...
        {
            XmlNodeList xnList = XmlHelper.ReadNodes(dirPath + QJConst.FILE_PLUGIN_XML_CONFING, xPath);
            foreach (XmlElement xe in xnList)
            {
                if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite")
                {
                    ...
                    {
                        //生成模板文件
                        PageTemplate.GetTemplate(sitePath, tempPath, skinName, xe.Attributes["templet"].InnerText, xe.Attributes["page"].InnerText, xe.Attributes["inherit"].InnerText, QJConst.DIRECTORY_REWRITE_PLUGIN, "", 1);
                    }
                }
            }
        }
        ...
        {
            return XmlHelper.UpdateNodeInnerText(dirPath + QJConst.FILE_PLUGIN_XML_CONFING, xPath, value);
        }
        ...
        {
            bool result = true;
            List<string> ls = ReadChildNodesValue(dirPath + QJConst.FILE_PLUGIN_XML_CONFING, xPath);
            if (ls != null)
            {
                ...
                if (!DbHelperSQL.ExecuteSqlTran(al))
                {
                    result = false;
                }
            }
