...
namespace QJcms.Web.admin.channel
{
    public partial class category_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_channel_category", QJEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("site_channel_category", QJEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.channel_category bll = new BLL.channel_category();
            ...
            AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "保存频道分类排序"); //记录日志
        ...
        {
            ChkAdminLevel("site_channel_category", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.channel_category bll = new BLL.channel_category();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    {
                        sucCount += 1;
                        Utils.DeleteDirectory(siteConfig.webpath + QJConst.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                        Utils.DeleteDirectory(siteConfig.webpath + QJConst.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                    }
                    else
                    {
                        errorCount += 1;
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除频道分类成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
