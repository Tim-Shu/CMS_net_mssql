...
namespace QJcms.Web.admin.channel
{
    public partial class category_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            {
                //更新一下域名缓存
                CacheHelper.Remove(QJConst.CACHE_SITE_HTTP_DOMAIN);
                AddAdminLog(QJEnums.ActionEnum.Add.ToString(), "添加频道分类:" + model.title); //记录日志
                return true;
            }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                //更新一下域名缓存
                CacheHelper.Remove(QJConst.CACHE_SITE_HTTP_DOMAIN);
                AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "修改频道分类:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("site_channel_category", QJEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
