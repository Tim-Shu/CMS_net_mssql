...
namespace QJcms.Web.admin.settings
{
    public partial class nav_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("app_navigation_list", QJEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(QJEnums.NavigationEnum.System.ToString()); //绑定导航菜单
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                model.nav_type = QJEnums.NavigationEnum.System.ToString();
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(QJEnums.ActionEnum.Add.ToString(), "添加导航信息:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "修改导航信息:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == QJEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("app_navigation_list", QJEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("app_navigation_list", QJEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
