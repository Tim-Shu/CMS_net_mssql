    ...
    public partial class url_rewrite_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == QJEnums.ActionEnum.Edit.ToString())
            {
                ...
                this.urlName = QJRequest.GetQueryString("name");
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model))
            {
                AddAdminLog(QJEnums.ActionEnum.Add.ToString(), "添加URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(string _urlName)
        {
            ...
            if (bll.Edit(model))
            {
                AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "修改URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == QJEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_url_rewrite", QJEnums.ActionEnum.Edit.ToString()); //检查权限
            ...
            {
                ChkAdminLevel("site_url_rewrite", QJEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
