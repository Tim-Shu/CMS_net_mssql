    ...
    public partial class templet_list : Web.UI.ManagePage
    {
        ...
        private void MarkTemplates(string buildPath, string skinName)
        {
            ...
            foreach (Model.url_rewrite modelt in ls)
            {
                ...
                {
                    PageTemplate.GetTemplate(siteConfig.webpath, "templates", skinName, modelt.templet, modelt.page, modelt.inherit, buildPath, modelt.channel, 1);
                }
            }
        }
        #endregion
        ...
        {
            ChkAdminLevel("app_templet_list", QJEnums.ActionEnum.Build.ToString()); //检查权限
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    AddAdminLog(QJEnums.ActionEnum.Build.ToString(), "生成模板:" + skinName);//记录日志
