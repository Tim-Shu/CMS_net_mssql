<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="user_list.aspx.cs" Inherits="QJcms.Web.admin.users.user_list" %>
...
<body class="mainbody">
    ...
    <!--导航栏-->
    <div class="location">
        <a href="javascript:history.back(-1);" class="back"><i></i><span>返回上一页</span></a> <a href="../center.aspx" class="home"><i></i><span>首页</span></a> <i class="arrow"></i><span>会员管理</span> <i class="arrow"></i><span>用户列表</span>
    </div>
    <!--/导航栏-->
    <!--工具栏-->
    <div class="toolbar-wrap">
        <div id="floatHead" class="toolbar">
            <div class="l-list">
                <ul class="icon-list">
                    <li><a class="add" href="user_edit.aspx?action=<%=QJEnums.ActionEnum.Add %>"><i></i><span>新增</span></a></li>
                    <li runat="server" id="tool_sms">
                ...
                <div class="menu-list" runat="server" id="divGroup">
                    <div class="rule-single-select">
                        <asp:DropDownList ID="ddlGroupId" runat="server" AutoPostBack="True" OnSelectedIndexChanged="ddlGroupId_SelectedIndexChanged">
                        </asp:DropDownList>
                    </div>
                </div>
            </div>
    ...
    <asp:Repeater ID="rptList" runat="server">
        ...
        <ItemTemplate>
            <tr>
                ...
                <td width="64">
                    <a href="user_edit.aspx?action=<%#QJEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">
                ...
                <td>
                    <div class="user-box">
                        <h4>
                            <b>
                                <%#Eval("user_name")%></b> (昵称：<%#Eval("nick_name")%>)</h4>
                        <i>注册时间：<%#string.Format("{0:g}",Eval("reg_time"))%></i> <span>
                            <%if (siteConfig.memberamountstatus == 1)
                              { %>
                ...
                <%if (siteConfig.membergroupstatus == 1)
                  { %>
                <td>
                    <%#GetUserGroup(Convert.ToInt32(Eval("group_id")))%>
                </td>
                <%}%>
                <td>
                    <%#Eval("email")%>
                ...
                <%}%>
                <td align="center">
                    <%#GetUserStatus(Convert.ToInt32(Eval("status")))%>
                </td>
                <td align="center">
                    <a href="user_edit.aspx?action=<%#QJEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">修改</a>
                </td>
            </tr>
        </ItemTemplate>
        <FooterTemplate>
