...
namespace QJcms.Web.admin.users
{
    public partial class user_sms : Web.UI.ManagePage
    {
        ...
        {
            mobiles = QJRequest.GetString("mobiles");
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_sms", QJEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo(mobiles);
        ...
        {
            ChkAdminLevel("user_sms", QJEnums.ActionEnum.Add.ToString()); //检查权限
            //检查短信内容
            ...
            if (rblSmsType.SelectedValue == "1")
            {
                ...
                if (result)
                {
                    AddAdminLog(QJEnums.ActionEnum.Add.ToString(), "发送手机短信"); //记录日志
