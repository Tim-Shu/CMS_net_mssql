...
namespace QJcms.Web.admin.users
{
    public partial class message_list : Web.UI.ManagePage
    {
        ...
        {
            this.type_id = QJRequest.GetQueryInt("type_id");
        ...
        {
            ChkAdminLevel("user_message", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_message bll = new BLL.user_message();
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除站内短消息成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
