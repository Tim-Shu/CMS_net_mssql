...
namespace QJcms.Web.admin.users
{
    public partial class oauth_app_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_oauth", QJEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("user_oauth", QJEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.user_oauth_app bll = new BLL.user_oauth_app();
            ...
            AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "保存OAuth配置排序"); //记录日志
        ...
        {
            ChkAdminLevel("user_oauth", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_oauth_app bll = new BLL.user_oauth_app();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除OAuth配置成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
