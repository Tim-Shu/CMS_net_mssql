...
namespace QJcms.Web.admin.users
{
    public partial class message_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == QJEnums.ActionEnum.View.ToString())
            {
                this.action = QJEnums.ActionEnum.View.ToString();//修改类型
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_message", QJEnums.ActionEnum.View.ToString()); //检查权限
                if (action == QJEnums.ActionEnum.View.ToString()) //修改
                {
                    ShowInfo(this.id);
                }
            }
        ...
        {
            ChkAdminLevel("user_message", QJEnums.ActionEnum.Add.ToString()); //检查权限
            if (!DoAdd())
            {
                JscriptMsg("发送过程中发生错误！", "", "Error");
                return;
            }
            AddAdminLog(QJEnums.ActionEnum.Add.ToString(), "发送站内短消息"); //记录日志
