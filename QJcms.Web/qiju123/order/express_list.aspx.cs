...
namespace QJcms.Web.admin.order
{
    public partial class express_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_express", QJEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("order_express", QJEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.express bll = new BLL.express();
            ...
            AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "保存配送方式排序"); //记录日志
        ...
        {
            ChkAdminLevel("order_express", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.express bll = new BLL.express();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除配送方式成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
