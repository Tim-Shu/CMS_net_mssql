...
namespace QJcms.Web.admin.order
{
    public partial class order_list : Web.UI.ManagePage
    {
        ...
        {
            this.status = QJRequest.GetQueryInt("status");
            this.payment_status = QJRequest.GetQueryInt("payment_status");
            this.express_status = QJRequest.GetQueryInt("express_status");
            this.keywords = QJRequest.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_list", QJEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("order_list", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.orders bll = new BLL.orders();
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除订单成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
