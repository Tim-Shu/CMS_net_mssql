...
namespace QJcms.Web.admin.order
{
    public partial class order_confirm : Web.UI.ManagePage
    {
        ...
        {
            ChkAdminLevel("order_list", QJEnums.ActionEnum.Confirm.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.orders bll = new BLL.orders();
            ...
            AddAdminLog(QJEnums.ActionEnum.Confirm.ToString(), "确认订单成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
