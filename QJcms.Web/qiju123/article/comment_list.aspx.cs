...
namespace QJcms.Web.admin.article
{
    public partial class comment_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("channel_" + this.channel_name + "_comment", QJEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_comment", QJEnums.ActionEnum.Audit.ToString()); //检查权限
            BLL.article_comment bll = new BLL.article_comment();
            ...
            AddAdminLog(QJEnums.ActionEnum.Audit.ToString(), "审核" + this.channel_name + "频道评论信息"); //记录日志
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_comment", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.article_comment bll = new BLL.article_comment();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除" + this.channel_name + "频道评论成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
