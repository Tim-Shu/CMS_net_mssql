...
namespace QJcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        private void ChannelBind()
        {
            ...
            isSys = rolebll.Exists(rolemodel.role_id, "channel_" + this.channel_name + "_list", QJEnums.ActionEnum.System.ToString());
            if (!isSys)
            {
                dlSystem.Visible = false;
            }

            if (chanModel.deep_layer < 1) { dlCategory.Visible = false; }
            if (chanModel.is_cover == 0) { dlCover.Visible = false; }
            if (chanModel.recom_type.Length > 0)
            {
                string[] recomList = chanModel.recom_type.Split(',');
                foreach (string s in recomList)
                {
                    switch (s)
                    {
                        case "isMsg":
                            cblRecomItem.Items.Add(new ListItem("允许评论", "isMsg"));
                            break;
                        case "isTop":
                            cblRecomItem.Items.Add(new ListItem("置顶", "isTop"));
                            break;
                        case "isRed":
                            cblRecomItem.Items.Add(new ListItem("推荐", "isRed"));
                            break;
                        case "isHot":
                            cblRecomItem.Items.Add(new ListItem("热门", "isHot"));
                            break;
                        case "isSlide":
                            cblRecomItem.Items.Add(new ListItem("幻灯片", "isSlide"));
                            break;
                        default:
                            break;
                    }
            ...
            else { dlRecom.Visible = false; }
            if (siteConfig.memberpointstatus == 0)
            {
                div_point.Visible = false;
            }
            if (siteConfig.membergroupstatus == 0)
            {
                rptPrice.Visible = false;
            }
            if (chanModel.is_content_seo == 0) { tab_seo.Visible = false; div_Seo.Visible = false; }
            if (chanModel.is_content_link == 0) { dlLink.Visible = false; }
            if (chanModel.is_content_into == 0) { dlInto.Visible = false; }
            if (chanModel.is_content_call == 0) { dlCall.Visible = false; }
            else
            {
                txtTitle.Attributes.Add("onBlur", "change2cn(this.value, this.form.txtCallIndex)");
                if (action == QJEnums.ActionEnum.Add.ToString())
                {
                    txtCallIndex.Attributes.Add("ajaxurl", "../../tools/admin_ajax.ashx?action=article_call_validate");
                }
            }
                ...
                switch (modelt.control_type)
                {
                    case "single-text": //单行文本
                        ...
                        if (modelt.control_type == "images")
                        {
                            ...
                            //添加图片预览
                            HtmlGenericControl htmlView = new HtmlGenericControl("div");
                            htmlView.Attributes.Add("class", "view-img-box");

                            HtmlGenericControl htmlImg = new HtmlGenericControl("img");
                            htmlImg.Attributes.Add("class", "img-view");
                            htmlImg.Attributes.Add("src", "../skin/default/loadimg.gif");
                            htmlView.Controls.Add(htmlImg);
                            htmlDD.Controls.Add(htmlView);
                        }
                    ...
                    case "multi-checkbox": //多项多选
                        ...
                        break;
                    case "data-property": //属性规格
                        HiddenField hidControls = new HiddenField();
                        hidControls.ID = "field_control_" + modelt.name;
                        HtmlGenericControl htmlDiv4 = new HtmlGenericControl("div");
                        htmlDiv4.Attributes.Add("class", "data-property rule-data-property");
                        htmlDiv4.Attributes.Add("data", modelt.name);
                        //htmlDiv4.InnerHtml = "<ul id=\"field_prop_" + modelt.name + "\"></ul>";
                        //htmlDiv4.InnerHtml += "<a class=\"add-prop\" href=\"javascript:void(0);\" data=\"" + modelt.name + "\"><i></i><span>添加</span></a>";
                        htmlDD.Controls.Add(htmlDiv4);
                        htmlDD.Controls.Add(hidControls);
                        break;
                }
        ...
        private void ShowSysField(int _channel_id)
        {
            ...
            {
                if (siteConfig.membergroupstatus == 1)
                {
                    GroupBind(""); //绑定用户组
                }
                else
        ...
        private void ShowInfo(int _id)
        {
            ...
            if (txtCallIndex.Text.Trim() != "")
            {
                txtCallIndex.Enabled = false;
            }
            if (model.is_sys == 1)
            {
            ...
            {
                if (strRecom.Contains(item.Value))
                {
                    item.Selected = true;
                }
            }
            ...
            foreach (Model.article_attribute_field modelt1 in ls1)
            {
                switch (modelt1.control_type)
                {
                    ...
                    case "data-property": //属性规格
                        ...
                        hidControl.Value = model.fields[modelt1.name];
                        break;
                }
            }
            #endregion
            //绑定图片相册 开启相册功能并关闭封面功能
            if (chanModel.is_albums == 1 && chanModel.is_cover == 0)
            {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                switch (dr["control_type"].ToString())
                {
                    ...
                    case "multi-checkbox": //多项多选
                        ...
                        break;
                    case "data-property":
                        HiddenField hidControl = FindControl("field_control_" + dr["name"].ToString()) as HiddenField;
                        if (hidControl != null)
                        {
                            dic.Add(dr["name"].ToString(), hidControl.Value.Trim());
                        }
                        break;
                }
            }
        ...
        private bool DoAdd()
        {
            ...
            #region 推荐类型
            if (chanModel.recom_type.Trim().Length > 0)
            {
                model.is_msg = 0;
                model.is_top = 0;
                model.is_red = 0;
            ...
            model.user_name = GetAdminInfo().real_name;// "admin"; //获得当前登录用户名
            model.add_time = Utils.StrToDateTime(txtAddTime.Text.Trim());
            model.fields = SetFieldValues(this.channel_id); //扩展字段赋值

            #region 保存相册====================
            if (chanModel.is_albums == 1)
            {
                //检查是否有自定义图片
                if (txtImgUrl.Text.Trim() == "" && chanModel.is_cover == 0)
                {
            ...
            if (bll.Add(model) > 0)
            {
                ...
                AddAdminLog(QJEnums.ActionEnum.Add.ToString(), "添加" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (chanModel.is_albums == 1)
            {
                model.img_url = hidFocusPhoto.Value;
                //检查是否有自定义图片
                if (txtImgUrl.Text.Trim() != ""&&chanModel.is_cover==1)
                {
                    model.img_url = txtImgUrl.Text;
                }
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "修改" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == QJEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", QJEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", QJEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
