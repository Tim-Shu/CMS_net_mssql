...
{
    public partial class serv_list : Web.UI.ManagePage
    {
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("serv_list.aspx", "group_id={0}&keywords={1}", this.group_id.ToString(), txtKeywords.Text));
        }

        //筛选分组
        protected void ddlGroupId_SelectedIndexChanged(object sender, EventArgs e)
        {
            Response.Redirect(Utils.CombUrlTxt("serv_list.aspx", "group_id={0}&keywords={1}", ddlGroupId.SelectedValue, this.keywords));
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "修改在线客服排序"); //记录日志
            JscriptMsg("保存排序成功！", Utils.CombUrlTxt("serv_list.aspx", "group_id={0}&keywords={1}", this.group_id.ToString(), this.keywords), "Success");
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("tools_server", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.online_service bll = new BLL.online_service();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除在线客服成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！",
                Utils.CombUrlTxt("serv_list.aspx", "group_id={0}&keywords={1}", this.group_id.ToString(), this.keywords), "Success");
        }
    }
}