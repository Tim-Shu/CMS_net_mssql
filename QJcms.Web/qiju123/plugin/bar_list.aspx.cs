...
namespace QJcms.Web.admin.plugin
{
    public partial class bar_list : Web.UI.ManagePage
    {
        ...
        private void BindPower()
        {
            ...
            bool result = rolebll.Exists(rolemodel.role_id, "tools_advert", QJEnums.ActionEnum.Add.ToString());
            if (!result)
            {
                tool_add.Visible = false;
            }
            result = rolebll.Exists(rolemodel.role_id, "tools_advert", QJEnums.ActionEnum.Edit.ToString());
            if (!result)
            {
                tool_edit.Visible = false;
            }
            result = rolebll.Exists(rolemodel.role_id, "tools_advert", QJEnums.ActionEnum.Delete.ToString());
            if (!result)
            {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(QJEnums.ActionEnum.Edit.ToString(), "修改广告内容排序"); //记录日志
        ...
        {
            ChkAdminLevel("tools_advert", QJEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.advert_banner bll = new BLL.advert_banner();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(QJEnums.ActionEnum.Delete.ToString(), "删除广告内容成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
