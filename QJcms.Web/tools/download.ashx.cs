...
namespace QJcms.Web.tools
{
    ...
    public class download : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            int id = QJRequest.GetQueryInt("id");
            //获得下载ID
            if (id < 1)
            {
            ...
            if (model.point > 0)
            {
                ...
                //防止重复扣积分
                string cookie = Utils.GetCookie(QJConst.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString());
                if (cookie != model.id.ToString())
                {
                    ...
                    //写入Cookie
                    Utils.WriteCookie(QJConst.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString(), model.id.ToString(), 8640);
                }
            }
            //下载次数+1
