...
namespace QJcms.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void UpLoadFile(HttpContext context)
        {
            ...
            string _delfile = QJRequest.GetString("DelFilePath");
            int channel_id = QJRequest.GetInt("channel_id", 0);
            HttpPostedFile _upfile = context.Request.Files["Filedata"];
            bool _iswater = false; //默认不打水印
            bool _isthumbnail = false; //默认不生成缩略图

            if (QJRequest.GetQueryString("IsWater") == "1")
                _iswater = true;
            if (QJRequest.GetQueryString("IsThumbnail") == "1")
                _isthumbnail = true;
            if (_upfile == null)
            {
            ...
            string msg = upFiles.fileSaveAs(_upfile, _isthumbnail, _iswater, channel_id);
            //删除已存在的旧文件
        ...
        private void EditorFile(HttpContext context)
        {
            ...
            UpLoad upFiles = new UpLoad();
            string remsg = upFiles.fileSaveAs(imgFile, false, _iswater, channel_id);
