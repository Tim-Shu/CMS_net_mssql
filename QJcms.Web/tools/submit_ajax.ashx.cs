...
namespace QJcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "view_cart_count": //输出当前购物车总数
                    ...
                    break;
                case "view_province_list": //输出配送省份列表
                    view_province_list(context);
                    break;
                case "view_city_list": //输出配送城市列表
                    view_city_list(context);
                    break;
                case "view_district_list": //输出配送区域列表
                    view_district_list(context);
                    break;
        ...
        private void comment_add(HttpContext context)
        {
            ...
            //string code = QJRequest.GetFormString("txtCode");
            int article_id = QJRequest.GetQueryInt("article_id");
            string _content = QJRequest.GetFormString("txtContent");
            int order_goods_id = QJRequest.GetQueryInt("order_goods_id");
            ////校检验证码
            ...
            model.user_ip = QJRequest.GetIP();
            model.is_lock = siteConfig.commentstatus; //审核开关
            model.add_time = DateTime.Now;
            model.is_reply = 0;
            int comment_id = bll.Add(model);
            if (comment_id > 0)
            {
                new BLL.orders().UpdateOrderGoodField(order_goods_id, "comment_id=" + comment_id);
        ...
        {
            int article_id = QJRequest.GetQueryInt("article_id");
            int page_index = QJRequest.GetQueryInt("page_index");
            int page_size = QJRequest.GetQueryInt("page_size");
            int totalcount;
            StringBuilder strTxt = new StringBuilder();
        ...
        {
            string username = QJRequest.GetString("param");
            //如果为Null，退出
        ...
        {
            string code = QJRequest.GetFormString("txtCode").Trim();
            string invitecode = QJRequest.GetFormString("txtInviteCode").Trim();
            string username = Utils.ToHtml(QJRequest.GetFormString("txtUserName").Trim());
            string password = QJRequest.GetFormString("txtPassword").Trim();
            string password2 = QJRequest.GetFormString("txtPassword2").Trim();
            string email = Utils.ToHtml(QJRequest.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(QJRequest.GetFormString("txtMobile").Trim());
            string userip = QJRequest.GetIP();
            ...
            if (string.IsNullOrEmpty(username))
            {
                context.Response.Write("{ \"status\":0, \"msg\":\"用户名不可为空！\"}");
                return;
            }
            if (username.Length < 3 || username.Length > 20)
            {
                context.Response.Write("{ \"status\":0, \"msg\":\"用户名合法长度为3-20位字母、数字或下划线组合！\"}");
                return;
            }
            //过滤注册用户名字符
            string[] strArray = userConfig.regkeywords.Split(',');
            foreach (string s in strArray)
            {
                if (s.ToLower() == username.ToLower())
                {
                    context.Response.Write("{ \"status\":0, \"msg\":\"该用户名不可用！\"}");
                    return;
                }
            }
            BLL.users bll = new BLL.users();
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"错误：密码不能为空！\"}");
                return;
            }
            if (!password.Equals(password2))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"错误：两次密码输入不一致！\"}");
                return;
            }
            ...
            if (siteConfig.smsstatus == 1 && userConfig.mobilelogin == 1 && mobile == "")
            {
            ...
            //不允许同一手机号码注册不同用户
            if (siteConfig.smsstatus == 1 && userConfig.mobilelogin == 1)
            {
            ...
            if (modelGroup != null)
            {
                ...
                {
                    new BLL.user_amount_log().Add(model.id, model.user_name, QJEnums.AmountTypeEnum.SysGive.ToString(), modelGroup.amount, "注册赠送金额", 1);
                }
                bll.UpdateField(model.id, "group_id=" + modelGroup.id);
            }
            #endregion
            ...
            {
                context.Session[QJConst.SESSION_USER_INFO] = model;
                context.Session.Timeout = 45;

                //防止Session提前过期
                Utils.WriteCookie(QJConst.COOKIE_USER_NAME_REMEMBER, "QJcms", model.user_name);
                Utils.WriteCookie(QJConst.COOKIE_USER_PWD_REMEMBER, "QJcms", model.password);
        ...
        private string verify_email(Model.users userModel)
        {
            ...
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, QJEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
                codeModel.user_name = userModel.user_name;
                codeModel.type = QJEnums.CodeEnum.RegVerify.ToString();
            ...
            //替换模板内容
            string titletxt = mailModel.mail_title;
            ...
            try
            {
                QJMail.sendMail(siteConfig.emailsmtp,
        ...
        private string verify_mobile(Model.users userModel)
        {
            ...
            Model.user_code codeModel;
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, QJEnums.CodeEnum.RegVerify.ToString(), "n");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + QJEnums.CodeEnum.Register.ToString() + "' and datediff(d,add_time,getdate())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + QJEnums.CodeEnum.Register.ToString() + "' and status=1 or datediff(d,eff_time,getdate())>0");
            //随机取得邀请码
            ...
            codeModel.type = QJEnums.CodeEnum.Register.ToString();
        ...
        private void user_register_smscode(HttpContext context)
        {
            ...
            //写入SESSION，保存验证码
            context.Session[QJConst.SESSION_SMS_CODE] = strcode;
        ...
        {
            string username = QJRequest.GetFormString("username");
            //检查是否过快
        ...
        {
            string username = QJRequest.GetFormString("txtUserName");
            string password = QJRequest.GetFormString("txtPassword");
            string code = QJRequest.GetFormString("txtCode").Trim();
            string remember = QJRequest.GetFormString("chkRemember");
            //检查用户名密码
            ...
            {
                Utils.WriteCookie(QJConst.COOKIE_USER_NAME_REMEMBER, "QJcms", model.user_name, 43200);
                Utils.WriteCookie(QJConst.COOKIE_USER_PWD_REMEMBER, "QJcms", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            //检查用户名密码
            string username = QJRequest.GetString("txtUserName");
            string password = QJRequest.GetString("txtPassword");
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"温馨提示：请输入用户名或密码！\"}");
                return;
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string email = Utils.ToHtml(QJRequest.GetFormString("txtEmail"));
            string nick_name = Utils.ToHtml(QJRequest.GetFormString("txtNickName"));
            string sex = QJRequest.GetFormString("rblSex");
            string birthday = QJRequest.GetFormString("txtBirthday");
            string telphone = Utils.ToHtml(QJRequest.GetFormString("txtTelphone"));
            string mobile = Utils.ToHtml(QJRequest.GetFormString("txtMobile"));
            string qq = Utils.ToHtml(QJRequest.GetFormString("txtQQ"));
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            string fileName = QJRequest.GetFormString("hideFileName");
            int x1 = QJRequest.GetFormInt("hideX1");
            int y1 = QJRequest.GetFormInt("hideY1");
            int w = QJRequest.GetFormInt("hideWidth");
            int h = QJRequest.GetFormInt("hideHeight");
            //检查是否图片
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string oldpassword = QJRequest.GetFormString("txtOldPassword");
            string password = QJRequest.GetFormString("txtPassword");
            //检查输入的旧密码
        ...
        {
            string code = QJRequest.GetFormString("txtCode");
            string username = QJRequest.GetFormString("txtUserName").Trim();
            //检查用户名是否正确
            if (string.IsNullOrEmpty(username))
            {
            ...
            BLL.user_code codeBll = new BLL.user_code();
            Model.user_code codeModel;
            codeModel = codeBll.GetModel(username, QJEnums.CodeEnum.RegVerify.ToString(), "'d'");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                //写入数据库
                codeModel.user_id = model.id;
                codeModel.user_name = model.user_name;
                codeModel.type = QJEnums.CodeEnum.Password.ToString();
                codeModel.str_code = strcode;
                codeModel.eff_time = DateTime.Now.AddDays(userConfig.regemailexpired);
                codeModel.add_time = DateTime.Now;
                codeBll.Add(codeModel);
        ...
        private void user_repassword(HttpContext context)
        {
            ...
            string password1 = context.Request.Form["txtPassword1"];
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"密码长度小于6位！\"}");
                return;
            }
            if (!password.Equals(password1))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"输入的两次密码不同！\"}");
                return;
            }
        ...
        private void user_message_delete(HttpContext context)
        {
            ...
            string check_id = QJRequest.GetFormString("checkId");
            if (check_id == "")
            {
        ...
        private void user_message_add(HttpContext context)
        {
            ...
            string code = context.Request.Form["txtCode"];
            string send_save = QJRequest.GetFormString("sendSave");
            string user_name = Utils.ToHtml(QJRequest.GetFormString("txtUserName"));
            string title = Utils.ToHtml(QJRequest.GetFormString("txtTitle"));
            string content = Utils.ToHtml(QJRequest.GetFormString("txtContent"));
            //校检验证码
            string result = verify_code(context, code);
            if (result != "success")
            {
        ...
        private void user_point_convert(HttpContext context)
        {
            ...
            int amout = QJRequest.GetFormInt("txtAmount");
            string password = QJRequest.GetFormString("txtPassword");
            if (model.amount < 1)
            {
            ...
            //扣除金额
            int amountNewId = new BLL.user_amount_log().Add(model.id, model.user_name, QJEnums.AmountTypeEnum.Convert.ToString(), amout * -1, "用户兑换积分", 1);
            //增加积分
            if (amountNewId < 1)
            {
            ...
            {
                //返还金额
                new BLL.user_amount_log().Add(model.id, model.user_name, QJEnums.AmountTypeEnum.Convert.ToString(), amout, "用户兑换积分失败，返还金额", 1);
                context.Response.Write("{\"status\":0, \"msg\":\"转换过程中发生错误，请重新提交！\"}");
                return;
            }
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            decimal amount = QJRequest.GetFormDecimal("order_amount", 0);
            int payment_id = QJRequest.GetFormInt("payment_id");
            if (amount == 0)
            {
            ...
            new BLL.user_amount_log().Add(model.id, model.user_name, QJEnums.AmountTypeEnum.Recharge.ToString(), order_no,
        ...
        {
            string goods_id = QJRequest.GetFormString("goods_id");
            int goods_quantity = QJRequest.GetFormInt("goods_quantity", 1);
            decimal goods_price = QJRequest.GetFormDecimal("goods_price", 0);
            if (goods_id == "")
            {
            ...
            #region 检验产品价格（防止攻击）
            string propertyStr = "";
            Model.article model = new BLL.article().GetModel(int.Parse(goods_id));
            if (model != null && model.fields.ContainsKey("property"))
            {
                propertyStr = model.fields["property"];
            }
            ...
            {
                string[] proplist = propertyStr.Split(',');
            ...
            {
                if (goods_price == Utils.StrToDecimal(price["value"].ToString(), 0))
                {
                    rightPrice = true; break;
                }
            }
            #endregion

            if (goods_price <= 0 || !rightPrice)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品价格有误！\"}");
                return;
            }
            //查找会员组
            int group_id = 0;
            ...
            //统计购物车
            Web.UI.ShopCart.Add(goods_id + "|" + goods_price, goods_quantity);
        ...
        {
            string goods_id = QJRequest.GetFormString("goods_id");
            int goods_quantity = QJRequest.GetFormInt("goods_quantity");
            if (goods_id == "")
            {
                context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品参数有误！\"}");
                return;
        ...
        {
            #region 原版商城保存购物 2015-03-18 增加积分抵扣金额 删除配送方式
            //获得传参信息
            int payment_id = QJRequest.GetFormInt("payment_id");
            int express_id = QJRequest.GetFormInt("express_id");
            string accept_name = Utils.ToHtml(QJRequest.GetFormString("accept_name"));
            string post_code = Utils.ToHtml(QJRequest.GetFormString("post_code"));
            string telphone = Utils.ToHtml(QJRequest.GetFormString("telphone"));
            string mobile = Utils.ToHtml(QJRequest.GetFormString("mobile"));
            string address = Utils.ToHtml(QJRequest.GetFormString("address"));
            string message = Utils.ToHtml(QJRequest.GetFormString("message"));

            //新增城市列表 2015-03-18
            string province = Utils.ToHtml(QJRequest.GetFormString("province"));
            string city = Utils.ToHtml(QJRequest.GetFormString("city"));
            string district = Utils.ToHtml(QJRequest.GetFormString("district"));
            int point_cost = QJRequest.GetFormInt("cost_point", 0);
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = QJcms.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请选择城市区域！\"}");
                return;
            }
            ...
            //新增积分抵扣金额 检查积分  2015-03-18
            if (point_cost > 0)
            {
                if (point_cost > userModel.point)
                {
                    context.Response.Write("{\"status\":0, \"msg\":\"对不起，用户当前积分少于兑换额度！\"}");
                    return;
                }
            }

            //统计购物车
            Model.cart_total cartModel = QJcms.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            ...
            model.telphone = telphone;
            model.mobile = mobile;
            model.area = province + "," + city + "," + district;
            model.address =address;
            ...
            model.order_amount = model.real_amount + model.express_fee + model.payment_fee - ((Decimal)(point_cost * 0.01));
            //购物积分,可为负数
            model.point_cost = point_cost;
            ...
            {
                gls.Add(new Model.order_goods { goods_id = item.id, goods_title = item.title, property_index = item.property_index, goods_price = item.price, real_price = item.user_price, quantity = item.quantity, point = item.point });
            }
            ...
            if (point_cost > 0)
            {
                new BLL.user_point_log().Add(model.user_id, model.user_name, (0 - point_cost), "积分抵扣，订单号：" + model.order_no, false);
            }
            //清空购物车
            QJcms.Web.UI.ShopCart.Clear("0");
            //提交成功，返回URL
        ...
        private void order_cancel(HttpContext context)
        {
            ...
            //检查订单是否存在
            string order_no = QJRequest.GetQueryString("order_no");
        ...
        {
            int article_id = QJRequest.GetInt("id", 0);
            int click = QJRequest.GetInt("click", 0);
            int count = 0;
            if (article_id > 0)
            {
        ...
        {
            int attach_id = QJRequest.GetInt("id", 0);
            int count = 0;
            if (attach_id > 0)
            {
        ...
        #region 输出配送省份列表OK===============================
        private void view_province_list(HttpContext context)
        {
            StringBuilder strOption = new StringBuilder();
            DataTable dt = new BLL.province().GetList(0, "is_lock=0", "").Tables[0];
            strOption.Append("<option value=\"\" selected=\"selected\">---请选择---</option>");
            foreach (DataRow dr in dt.Rows)
            {
                strOption.Append("<option value=\"").Append(dr["id"]).Append("\">").Append(dr["province_name"]).Append("</option>");
            }
        ...
        #region 输出配送城市列表OK===============================
        private void view_city_list(HttpContext context)
        {
            int province_id = QJRequest.GetInt("province_id", 0);
            StringBuilder strOption = new StringBuilder();
            DataTable dt = new BLL.city().GetList(0, "is_lock=0 and province_id=" + province_id, "").Tables[0];
            strOption.Append("<option value=\"\" selected=\"selected\">---请选择---</option>");
            foreach (DataRow dr in dt.Rows)
            {
                strOption.Append("<option value=\"").Append(dr["id"]).Append("\">").Append(dr["city_name"]).Append("</option>");
            }
            context.Response.Write(strOption.ToString());
        }
        #endregion
        ...
        #region 输出配送区域列表OK===============================
        private void view_district_list(HttpContext context)
        {
            int city_id = QJRequest.GetInt("city_id", 0);
            StringBuilder strOption = new StringBuilder();
            DataTable dt = new BLL.district().GetList(0, "is_lock=0 and city_id=" + city_id, "").Tables[0];
            strOption.Append("<option value=\"\" selected=\"selected\">---请选择---</option>");
            foreach (DataRow dr in dt.Rows)
            {
                strOption.Append("<option value=\"").Append(dr["id"]).Append("\">").Append(dr["district_name"]).Append("</option>");
            }
            context.Response.Write(strOption.ToString());
        }
        #endregion
        ...
        private string verify_code(HttpContext context, string strcode)
        {
            ...
            if (strcode.ToLower() != (context.Session[QJConst.SESSION_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[QJConst.SESSION_CODE] = null;
            return "success";
        }
        //校检网站验证码
        private string verify_sms_code(HttpContext context, string strcode)
        {
            ...
            if (context.Session[QJConst.SESSION_SMS_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[QJConst.SESSION_SMS_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[QJConst.SESSION_SMS_CODE] = null;
            return "success";
        }
        #endregion END通用方法=================================================
