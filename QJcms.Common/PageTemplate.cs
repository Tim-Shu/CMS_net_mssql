...
namespace QJcms.Common
{
    ...
    public abstract class PageTemplate
    {
        ...
        public static string GetTemplate(string sitePath, string tempPath, string skinName, string templet, string fromPage, string inherit, string buildPath, string channelName, int nest)
        {
            ...
            using (StreamReader objReader = new StreamReader(templetFullPath, Encoding.UTF8))
            {
                ...
                for (int i = 0; i < strlist.Length; i++)
                {
                    if (strlist[i] == "")
                        continue;
                    strReturn.Append(ConvertTags(nest, sitePath, tempPath, skinName, strlist[i], channelName)); //搜索替换标签
                }
                ...
                if (nest == 1)
                {
                    ...
                    {
                        channelStr = "\r\n\tconst string channel_name = \"" + channelName + "\";";
                    }
                    //页面头部声明
        ...
        /// <param name="channelName">频道名称</param>
        /// <returns></returns>
        private static string ConvertTags(int nest, string sitePath, string tempPath, string skinName, string inputStr, string channelName)
        {
            string strReturn = "";
            ...
            foreach (Match m in r[0].Matches(strTemplate))
            {
                ...
                {
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "\r\n" + GetTemplate(sitePath, "templates", m.Groups[3].ToString(), m.Groups[4].ToString(), "", "", "",channelName, nest + 1) + "\r\n");
                }
                else
                {
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "\r\n" + GetTemplate(sitePath, tempPath, skinName, m.Groups[4].ToString(), "", "", "",channelName, nest + 1) + "\r\n");
                }
            }
            #endregion
            ...
            {
                if (IsCodeLine)
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "QJRequest.GetString(\"" + m.Groups[2] + "\")");
                else
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), string.Format("\" + QJRequest.GetString(\"{0}\") + \"", m.Groups[2]));
            }
            #endregion
